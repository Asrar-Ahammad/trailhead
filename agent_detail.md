...
- Icons: Phosphor has a Flutter package (`phosphor_flutter`) — keep using it, consistent with original icon choice
...
- No emojis anywhere in UI. Not in buttons, labels, toasts, notifications, AI-generated text, empty states. Text and icons (Phosphor) only. Strip emojis from AI outputs before render if model adds them despite instruction.
...
- **You — Records sub-tab**: PR leaderboard cards (100m/1k/5k/10k/half/marathon, longest run, longest duration, max elevation).
...
### You tab
Progress / Activities / Records sub-tabs. Records: PR leaderboard cards. Settings (gear icon): theme toggle, units, streak rest-day config, UI Sounds toggle.
...
**PR leaderboard — build spec (unchanged logic from original plan):**

Standalone-distance PRs (100m/1k/5k/10k/half/marathon):
- For runs matching category distance ± 2% tolerance, record time directly

Best-effort segment PRs:
- For every completed run, sliding-window scan over cumulative distance array: find minimum-time window matching each target distance (100m, 1k, 5k, 10k)
- Two-pointer over points sorted by timestamp/cumulative distance. Expand window until target distance reached, record elapsed time, slide start pointer forward, repeat. O(n) per category per run.
- Only run this scan for runs long enough to contain the segment
- Compare result against existing PersonalRecord rows for that category. Insert if beats rank 3, re-rank top 3, evict 4th

Other record categories (single value): longest run, longest duration, highest elevation gain — direct comparison against existing top 3 on run completion.

Trigger record check: server-side, in the points-insert route handler, after all points saved (unchanged — this logic lives in the backend regardless of client platform).

UI: leaderboard screen, one card per category, top 3 with rank, run date link. In-app notification/toast on run completion if new PR set: "New 5k PR! 24:12 (previous: 24:45)"
...
Sections, in order:
- Header: "Weekly Summary" + date range. Phosphor trophy/chart icon, no emoji.
...
**Direction: modern Strava-inspired base + 90s pixel accent layer.** Layout, spacing, color system, and card/button shapes stay clean and modern (legibility and usability come first — this is a fitness app people glance at mid-run). The retro/pixel treatment is applied deliberately on top: nav icons, stat card icons, badges, achievement moments, and short punchy UI labels get the pixel treatment (Pixelify Sans + custom pixel icon set). Body text, hero stat numbers, form fields, and AI-generated copy stay in the modern faces — those need to stay easily/quickly readable, not stylized. See Typography and Iconography subsections below for the exact split.
...
### Iconography — dual icon system
Base modern set + retro pixel set, applied by context:
- **Pixel icon set** (custom 16x16 or 24x24 grid, hard-edged, no anti-aliasing): used for bottom nav icons, stat card icons (distance, pace, elevation, calories, stride, cadence icons), streak flame, trophy/medal/PR icons, achievement badges. These are the frequently-seen, personality-carrying icons — the right place for the retro identity to actually register. Build as an SVG set snapped to a pixel grid (design tool: draw at 16px/24px canvas, no smoothing) or source a maintained open pixel-icon library and adapt weight/style to match — don't hand-roll every icon from scratch if a decent base set exists to modify.
- **Phosphor Icons** (existing modern set): kept for utility/functional icons where clarity matters more than character — settings gear, close/dismiss (X), back arrows, form field icons, chevrons, search. These appear constantly and briefly; pixelating them would add visual noise without adding value. `regular` weight default, `fill` weight for active/selected states.
- Standard sizes: pixel icons at 16px/24px/32px (multiples that stay crisp on a pixel grid — avoid arbitrary sizes like 20px or 28px that force sub-pixel scaling and blur the pixelation), Phosphor icons at 20px/24px/32px as before.
- Icon color always follows text-secondary token unless conveying accent/status (PR, success, warning, error use their respective semantic color) — applies to both icon sets equally, the retro layer doesn't get its own color rules.
- Rendering: ensure pixel icons are set to nearest-neighbor scaling (`FilterQuality.none` on the relevant `Image`/`SvgPicture` widget), not the default smooth interpolation — smoothing a pixel-art asset defeats the entire point and makes it look like a blurry mistake rather than an intentional style.
...
- **Stat card** (Home workout stats, run detail grid): surface background, 12px radius, `md` padding, label (retro label style, Pixelify Sans, text-secondary, uppercase) above value (Display medium or Body large depending on hierarchy, Barlow Condensed), pixel icon top-right
- **Bottom nav**: 3 items fixed (Home/Record/You), pixel icons throughout (filled/solid variant for active tab + accent color + small dot indicator beneath, matches existing built screens), inactive items text-secondary outline pixel icon variant, nav labels in Pixelify Sans retro label style
- **Bottom sheet**: 20px top-corner radius, drag handle bar centered at top, backdrop scrim 60% opacity black regardless of theme
- **Chip/badge** (streak flame, PR rank): pill shape, pixel icon + Pixelify Sans label, accent-muted background with accent icon/text in default state, full accent background for emphasized state (e.g. new PR)
...
### Constraints
- No emoji, per existing rule — Phosphor icons only
